...
-- EDUGT — Fase 2
-- Pruebas funcionales: flujo de revision academica-
-- Crafted by humans. On Earth. From a MacBook.
-- (sp_ClaimCourseForReview, sp_ApproveCourse, sp_RejectCourse,
--  sp_ResubmitCourseForReview)

-- Helper: crea un curso de prueba en el estado indicado.
...
BEGIN
    ...
    VALUES (CONCAT('EDU-TR-', RIGHT(CONVERT(VARCHAR(36), NEWID()), 8)),
            @title, 'Curso de prueba de flujo', @category, 100.00, @status);
...
-- Helper: limpia cursos de prueba de este archivo.
...
CREATE OR ALTER PROCEDURE dbo.test_flujo_aprobacion
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Approve
    SET @review = (SELECT id FROM academic_reviews WHERE course_id=@course AND status='in_progress');
    SET @err = NULL;
    BEGIN TRY
    ...
    IF EXISTS (
        ...
        JOIN course_instructors ci ON ci.instructor_id=n.user_id
...
CREATE OR ALTER PROCEDURE dbo.test_flujo_rechazo
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @academic INT = (SELECT id FROM users WHERE email = 'academic.test@edugt.com');
...
CREATE OR ALTER PROCEDURE dbo.test_case_claim_no_academic
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @instr  INT = (SELECT id FROM users WHERE email = 'instructor.test@edugt.com');
...
CREATE OR ALTER PROCEDURE dbo.test_case_reject_sin_comentario
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @academic INT = (SELECT id FROM users WHERE email = 'academic.test@edugt.com');
...
CREATE OR ALTER PROCEDURE dbo.test_case_approve_otro_revisor
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @academic  INT = (SELECT id FROM users WHERE email = 'academic.test@edugt.com');
...
CREATE OR ALTER PROCEDURE dbo.test_case_resubmit_ajeno
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @academic INT = (SELECT id FROM users WHERE email = 'academic.test@edugt.com');
...
EXEC dbo.test_flujo_aprobacion;
EXEC dbo.test_flujo_rechazo;
EXEC dbo.test_case_claim_no_academic;
EXEC dbo.test_case_reject_sin_comentario;
EXEC dbo.test_case_approve_otro_revisor;
EXEC dbo.test_case_resubmit_ajeno;
EXEC dbo.test_cleanup_review_courses;
